...
- visionOS 26 Beta 4 Release Notes
...
The visionOS 26 SDK provides support for developing apps for Apple Vision Pro devices running visionOS 26 beta 4. The SDK comes bundled with Xcode 26, available from the Mac App Store. For information on the compatibility requirements for Xcode 26, see [Xcode 26 Release Notes](https://developer.apple.com/documentation/Xcode-Release-Notes/xcode-26-release-notes).
...
- In visionOS 26 beta 4, accessory tracking fails and controllers are not recognized as spatial input devices. (156458710)
...
- EyeSight might not display content, even if an observer is present. (155800405)

  **Workaround:** Reboot the Apple Vision Pro.
...
- When using guided generation and tool calling together, some requests might lead to `LanguageModelSession.GenerationError.decodingFailure` due to a bug in constrained decoding. (153773704) (FB18190120)
- When using tool calling with guided generation, tools might not get called. (155313086) (FB18691470)

  **Workaround:** In `respond` or `streamResponse`, pass `includeSchemaInPrompt: false` and provide a few concrete examples of the output data structure as part of the prompt.
