...
### [Apps](https://developer.apple.com/documentation/watchos-release-notes/watchos-26-release-notes#Apps)
...
- If an [`accentColor`](https://developer.apple.com/documentation/swiftui/color/accentcolor) is not set, there is an issue on watchOS causing the buy button label in StoreKit views to appear the same, whether the component is enabled or disabled. (153218311)

  **Workaround:** Set an `accentColor` in your app Asset catalog or via the SwiftUI API [`.tint()`](https://developer.apple.com/documentation/swiftui/view/tint(_:)-93mfq).
...
- Workout Buddy might appear as unavailable in the watchOS Workout app after installing beta 1. (152691503)

  **Workaround:** Toggle Apple Intelligence off and on again on your paired iPhone to re-sync the state to Apple Watch and show Workout Buddy as available.