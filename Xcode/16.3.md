...
# Xcode 16.3 Beta 2 Release Notes

Update your apps to use new features, and test your apps against API changes.

## [Overview](https://developer.apple.com/documentation/xcode-release-notes/xcode-16_3-release-notes#Overview)

Xcode 16.3 beta 2 includes SDKs for iOS 18.4, iPadOS 18.4, tvOS 18.4, watchOS 11.4, macOS Sequoia 15.4, and visionOS 2.4. The Xcode 16.3 beta 2 release supports on-device debugging in iOS 15 and later, tvOS 15 and later, watchOS 7 and later, and visionOS. Xcode 16.3 beta 2 requires a Mac running macOS Sequoia 15.2 or later.
...
- Clipboard Viewer is not included with Additional Tools for Xcode 16.3 beta. (142986262)

  **Workaround:** Please download Additional Tools for Xcode 16.2.
...
- Foundation encoders/decoders user info dictionaries now require `Sendable` values. This may cause build errors even in the Swift 5 language mode in small edge cases where the userInfo property is set to a value like `init(myCustomInitializer:)` where the initializer is defined in an extension on `Dictionary` with a `Value == Any` constraint. (113200224)

  **Workaround:** Use syntax like `Dictionary(myCustomInitializer:)` instead of the `.init(...)`syntax when setting the userInfo property

- Passing Sendable Formatter subclasses across isolation boundaries in Swift may incorrectly cause compiler errors indicating that the type is not Sendable (144869425)

  **Workaround:** Annotate static/global properties of Sendable Formatter subclass types with nonisolated(unsafe), or wrap instances of Sendable subclasses in an @unchecked Sendable wrapper type to silence the error

### [Frameworks](https://developer.apple.com/documentation/xcode-release-notes/xcode-16_3-release-notes#Frameworks)

#### [Known Issues](https://developer.apple.com/documentation/xcode-release-notes/xcode-16_3-release-notes#Known-Issues)

- Using the `ManagedApp` framework with the the iOS or visionOS SDK will fail to compile. (145665850)
...
- Game Progress Manager is compatible with macOS and iOS. Additional platforms to be added at a later date. (144625754)
...
- Instruments Call Tree can occasionally cause crashes of the application. (146034535)
...
- A playground that imports a module from a shared workspace may fail to run. (143923367)

### [Simulator](https://developer.apple.com/documentation/xcode-release-notes/xcode-16_3-release-notes#Simulator)

#### [Resolved Issues](https://developer.apple.com/documentation/xcode-release-notes/xcode-16_3-release-notes#Resolved-Issues)

- Fixed: In order to use iPhone 16e in the simulator with Xcode 16.3 beta, download both the iPhone 16e Device Support and the iOS 18.3 Simulator Runtime from Xcode &gt; Settings &gt; Components. iPhone 16e is not currently compatible with iOS 18.4 beta. (143996070)
