...
- Fixed: Known issue: projects which have added support for the arm64e architecture may fail to build for the Simulator. To work around, remove arm64 from the project’s architectures or add the following build setting declaration:
...
- Xcodebuild may fail to find the downloaded metal toolchain when building a project with metal shaders by command line. (152810434)
  ...
  **Workaround:** Run the following commands for Xcode (build 17A5241e) to reregister the Metal Tolchain
  ...
  ```
   ...
   sed -i '' -e 's/17A5241c/17A5241e/g' /tmp/MyMetalExport/MetalToolchain-17A5241c.exportedBundle/ExportMetadata.plis
...
- The Enhanced Security capability options in Xcode to enable entitlements enforcing platform restrictions and read-only platform memory do not add those protections and — on iOS and visionOS — prevent the application from being installed. (152059695)

  **Workaround:** Manually remove the `com.apple.security.hardened-process`, `com.apple.security.hardened-process.dyld-ro`, `com.apple.security.hardened-process.enhanced-security-version`, `com.apple.security.hardened-process.hardened-heap`, and `com.apple.security.hardened-process.platform-restrictions` from your target’s entitlements file.
...
- Enabling the safe Swift/C++/C interoperation feature can cause a compilation error when used in a project with an older deployment target where Span isn’t available. (150740330)
  ...
  **Workaround:** Set your project’s minimum deployment target to iOS 19.0, macOS 16.0, tvOS 19.0, watchOS 12.0, or visionOS 3.0.
...
### [Swift Macro Build Performance](https://developer.apple.com/documentation/xcode-release-notes/xcode-26-release-notes#Swift-Macro-Build-Performance)
